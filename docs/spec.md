...
> Describes the tool as it is at **version 0.55.0** (`whl_colours_palette.html`, repo `~/Dev/Colours/Palette`, remote `github.com/wanghailei/colour`). History appears only as rationale. Sources: the current HTML file and the build-session transcripts (Claude Cowork session of 4 Jun – 27 Jul 2026, and Claude Code sessions of 27–29 Jul 2026).
...
- **Print references** (JSON only, 29 Jul 2026): each step carries a `print` entry beside its honest hex, in **one compact house format** — `cmyk` as `"C50 M12 Y39 K19"` (naive uncalibrated device conversion), always; `pt` as `"PC7621 PU8509"` — the nearest coated and nearest uncoated entries of an optional sidecar table, only when present (one substrate table yields one code). The table is `{ "coated": { "<code>": "#hex" }, "uncoated": { … } }` and loads two ways (0.55.0): **drop a `pantone.json` anywhere on the page** — it persists in this browser's `localStorage` and the licensed data never touches the public server — or, on private deployments, place it as a sidecar beside the HTML. It is licensed reference data, git-ignored, and never ships with the source; absence simply means exports carry no PT line — which is why the public whl.ltd exports have none until a table is dropped. Nearest match is by OKLab distance. Both values are approximations for print orientation only — the hex remains the truth (a match-distance disclosure was considered and declined: PT is a rough pointer by decision, not a certified equivalent). The Gradient tool reads `steps` alone, so `print` is invisible to it. sRGB stays hex everywhere.
- **SVG cell values** (29 Jul 2026, replacing the original no-text rule): every colour cell carries its full record as vector text in the cell's own ink — oklch title, upper-case hex, CMYK code, and PT code (only with the sidecar) — and the typography **mirrors the UI cell exactly**: one size (12), weight 400, left margin 16, line tops 12/31/50/69 (the UI's 19px pitch), hex and print lines at the UI's 0.78 ink. Every `<text>` element is self-contained (family, size, weight, spacing as its own attributes) because design tools ignore attributes inherited from groups — the lesson of 0.53.0, whose group-level sizing rendered wrong outside browsers. The font travels two ways (0.55.0): the woff2 itself is **embedded as a data URI** in a `<style>` block — a downloaded SVG has no server to resolve fonts from, and WebKit viewers (Safari, QuickLook) hide user-installed families, so the file must carry its own — while every text element also names the installed families, quoted, for the design tools that ignore embedded web fonts: `'Berkeley Mono Variable', 'Berkeley Mono', 'TX-02', Menlo, monospace`. The bytes come from the site's single woff2 copy, fetched at page load; on `file://` dev the fetch cannot run and the export relies on the names alone. The black and white frame plates stay bare.
...
- **29 Jul 2026 — 0.55.0.** The downloaded SVG made self-sufficient: the woff2 embeds as a data URI (WebKit viewers hide installed fonts and an offline file has no server, so named families alone kept falling to Menlo), alongside the named-family route for design tools. The PT table now loads by dropping `pantone.json` onto the page, persisting in `localStorage` — the answer to "why no PT on the public site": licensed data cannot be served publicly, so each owner drops their own.
