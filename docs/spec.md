...
> Describes the tool as it is at **version 0.55.1** (`whl_colours_palette.html`, repo `~/Dev/Colours/Palette`, remote `github.com/wanghailei/colour`). History appears only as rationale. Sources: the current HTML file and the build-session transcripts (Claude Cowork session of 4 Jun – 27 Jul 2026, and Claude Code sessions of 27–29 Jul 2026).
...
- **SVG cell values** (29 Jul 2026, replacing the original no-text rule): every colour cell carries its full record as vector text in the cell's own ink — oklch title, upper-case hex, CMYK code, and PT code (only with the sidecar) — and the typography **mirrors the UI cell exactly**: one size (12), weight 400, left margin 16, line tops 12/31/50/69 (the UI's 19px pitch), hex and print lines at the UI's 0.78 ink. Every `<text>` element is self-contained (family, size, weight, spacing as its own attributes) because design tools ignore attributes inherited from groups — the lesson of 0.53.0, whose group-level sizing rendered wrong outside browsers. The font is a plain CSS-style stack on every text element, nothing more — `'Berkeley Mono Variable', 'Berkeley Mono', 'IBM Plex Mono', 'JetBrains Mono', monospace` — set by the owner (0.55.1; a data-URI embedding shipped briefly in 0.55.0 and was withdrawn the same day as machinery: the viewer resolves whichever family it has). The black and white frame plates stay bare.
...
- **29 Jul 2026 — 0.55.0 / 0.55.1.** The PT table now loads by dropping `pantone.json` onto the page, persisting in `localStorage` — the answer to "why no PT on the public site": licensed data cannot be served publicly, so each owner drops their own. 0.55.0 also embedded the woff2 in SVG exports as a data URI; the owner withdrew it the same day (0.55.1) — no machinery, just the CSS-style stack `Berkeley Mono (Variable) → IBM Plex Mono → JetBrains Mono → monospace` on every text element.
