...
> Describes the tool as it is at **version 0.56.3** (`whl_colours_palette.html`, repo `~/Dev/Colours/Palette`, remote `github.com/wanghailei/colour`). History appears only as rationale. Sources: the current HTML file and the build-session transcripts (Claude Cowork session of 4 Jun – 27 Jul 2026, and Claude Code sessions of 27–29 Jul 2026).
...
- **Print references** (JSON and SVG, 29 Jul 2026): each step carries a `print` entry beside its honest hex, in **one compact house format** — `cmyk` as `"C50 M12 Y39 K19"` (naive uncalibrated device conversion), always; `pt` as `"PC186 PU1797"` — the nearest coated and nearest uncoated entries of the PT table, matched by OKLab distance. **The tool ships its own table** (0.56.0, the owner's call — supplying reference values is the tool's job): `pantone.json` deployed beside the served page, **1282 numeric codes per substrate** in `{ "coated": { "<code>": "#hex" }, "uncoated": { … } }` shape, sourced from the community `brettapeters/pantones` dataset, whose values match Pantone's published digital equivalents on every anchor checked (186 C = `#c8102e`, 485 C, Yellow C, Reflex Blue C, 300 C). **Numeric codes only** (0.56.2, the owner's rule): named colours (Reflex Blue, Neutral Black, Warm Gray N…) are stripped from the shipped table and filtered out of any dropped one — a PT reference without a number is not shown, and what does not correspond is not forced to correspond. Dropping a different `pantone.json` onto the page overrides it for this browser (`localStorage`). The table stays out of the Palette source repo (git-ignored); it lives with the deployment. Both values are approximations for print orientation only — the hex remains the truth (a match-distance disclosure was considered and declined: PT is a rough pointer by decision, not a certified equivalent). The Gradient tool reads `steps` alone, so `print` is invisible to it. sRGB stays hex everywhere.
- **SVG cell values** (29 Jul 2026, replacing the original no-text rule): every colour cell carries its full record as vector text in the cell's own ink — oklch title, upper-case hex, CMYK code, and PT code (only with the sidecar) — and the typography **mirrors the UI cell exactly**: one size (12), weight 400, left margin 16, line tops 12/31/50/69 (the UI's 19px pitch), hex and print lines at the UI's 0.78 ink. PC and PU share one line (0.56.3): numeric-only codes fit — the worst case, `PC9999 PU9999`, runs 110 of the cell's 150 units. (They were briefly split onto two lines in 0.56.1, when named codes still overflowed.) Every `<text>` element is self-contained (family, size, weight, spacing as its own attributes) because design tools ignore attributes inherited from groups — the lesson of 0.53.0, whose group-level sizing rendered wrong outside browsers. The font is a plain stack on every text element, written the way design apps' own SVG exports write it (0.56.0): **PostScript name first, family names after, no quotes anywhere** — `BerkeleyMonoVariable-Regular, Berkeley Mono Variable, Berkeley Mono, IBM Plex Mono, JetBrains Mono, monospace`. Affinity reads a quoted list as one literal unknown font name (the 0.55.1 defect) and matches PostScript names the way Sketch-exported SVGs expect; browsers skip the PostScript token and match the family names. `letter-spacing` was dropped from SVG text — importers choke on em units and 0.01em is sub-pixel at 12px. (A data-URI embedding shipped briefly in 0.55.0 and was withdrawn the same day as machinery.) The black and white frame plates stay bare.
...
- **29 Jul 2026 — 0.55.0 / 0.55.1.** The PT table gained a drop-to-load path persisting in `localStorage`. 0.55.0 also embedded the woff2 in SVG exports as a data URI; the owner withdrew it the same day (0.55.1) — no machinery, a plain font stack on every text element.
- **29 Jul 2026 — 0.56.0.** The tool now ships its own PT table — the owner's ruling ("supplying the values is the tool's job"): the community `brettapeters/pantones` dataset (1341 coated + 1341 uncoated, anchor-verified against Pantone's published values) deployed as `pantone.json` beside the served page; the drop path becomes an override. SVG font naming rewritten for design apps after Affinity failed to recognise the quoted list: PostScript name first, unquoted families after, letter-spacing dropped.
