...
> Describes the tool as it is at **version 0.53.0** (`whl_colours_palette.html`, repo `~/Dev/Colours/Palette`, remote `github.com/wanghailei/colour`). History appears only as rationale. Sources: the current HTML file and the build-session transcripts (Claude Cowork session of 4 Jun – 27 Jul 2026, and Claude Code sessions of 27–29 Jul 2026).
...
- **SVG cell values** (29 Jul 2026, replacing the original no-text rule): every colour cell carries its full record as vector text in the cell's own ink — oklch title (11 units), upper-case hex (10), CMYK code (9), and PT code (9, only with the sidecar) — stacked like a specimen label. The font is named, not embedded, leading with the installed family so design tools resolve it: `Berkeley Mono Variable, Berkeley Mono, TX-02, Menlo, monospace`; text stays selectable and editable. The black and white frame plates stay bare.
